(function($) {
	...
	var log = TestHelpers.menu.log, logOutput = TestHelpers.menu.logOutput, click = TestHelpers.menu.click;
	...
	test("handle click on menu",
			function() {
				...
				click($("#menu1"), "3");
				click(element, "1");
				equal(logOutput(), "click,1,afterclick,2,3,1",
						"Click order not valid.");
			});

	test("handle click on custom item menu",
			function() {
				expect(1);
	...
	asyncTest("handle blur", function() {
		expect(1);
		var blurHandled = false, element = $("#menu1").menu(
				{
					blur : function(event) {
						// Ignore duplicate blur event fired by IE
						if (!blurHandled) {
							blurHandled = true;
							equal(event.type, "menublur",
									"blur event.type is 'menublur'");
						}
					}
				});
	...
	asyncTest("handle focus of menu with active item", function() {
		...
		setTimeout(function() {
			...
			setTimeout(function() {
				element[0].focus();
				setTimeout(function() {
					equal(logOutput(), "focus,0,1,2,2",
							"current active item remains active");
					start();
				});
			});
		});
	...
	asyncTest("handle submenu auto collapse: mouseleave", function() {
		expect(4);
		var element = $("#menu2").menu(), event = $.Event("mouseenter");

		function menumouseleave1() {
			equal(element.find("ul[aria-expanded='true']").length, 1,
					"first submenu expanded");
			element.menu("focus", event, element
					.find("li:nth-child(7) li:first"));
			setTimeout(menumouseleave2, 350);
		}
		function menumouseleave2() {
			equal(element.find("ul[aria-expanded='true']").length, 2,
					"second submenu expanded");
			element.find("ul[aria-expanded='true']:first")
					.trigger("mouseleave");
			setTimeout(menumouseleave3, 350);
		}
		function menumouseleave3() {
			equal(element.find("ul[aria-expanded='true']").length, 1,
					"second submenu collapsed");
			element.trigger("mouseleave");
			setTimeout(menumouseleave4, 350);
		}
		function menumouseleave4() {
			equal(element.find("ul[aria-expanded='true']").length, 0,
					"first submenu collapsed");
			start();
		}
	...
	asyncTest("handle submenu auto collapse: mouseleave", function() {
		...
		function menumouseleave1() {
			equal(element.find("div[aria-expanded='true']").length, 1,
					"first submenu expanded");
			element.menu("focus", event, element.find(":nth-child(7)").find(
					"div").eq(0).children().eq(0));
			setTimeout(menumouseleave2, 350);
		}
		function menumouseleave2() {
			equal(element.find("div[aria-expanded='true']").length, 2,
					"second submenu expanded");
			element.find("div[aria-expanded='true']:first").trigger(
					"mouseleave");
			setTimeout(menumouseleave3, 350);
		}
		function menumouseleave3() {
		...
		function menumouseleave4() {
			equal(element.find("div[aria-expanded='true']").length, 0,
					"first submenu collapsed");
			start();
		}
	...
	asyncTest(
			...
			function() {
				...
				var element = $("#menu1").menu(
						{
							...
							focus : function(event) {
								log($(event.target).find(".ui-state-focus")
										.parent().index());
							}
						});
				...
				setTimeout(function() {
					...
					equal(logOutput(), "keydown",
							"Keydown PAGE_DOWN (no effect)");
	...
	asyncTest(
			"handle keyboard navigation on menu without scroll and with submenus",
			function() {
				expect(16);
				...
				function menukeyboard1() {
					...
					setTimeout(function() {
						equal(logOutput(), "keydown,1,2,3,4,0",
								"Keydown RIGHT (open submenu)");
				...
				function menukeyboard2() {
					...
					equal(logOutput(), "keydown,4",
							"Keydown LEFT (close submenu)");
				...
				function menukeyboard4() {
					equal(logOutput(), "keydown,0",
							"Keydown SPACE (open submenu)");
							...
							function() {
								...
								setTimeout(
										function() {
											...
											equal(logOutput(),
													"keydown,5,6,0,1,0,2,4,0",
													"Keydown skip dividers and items without anchors");
				...
				function menukeyboard6() {
					equal(logOutput(), "keydown,Ada",
							"Keydown ENTER (open submenu)");
					start();
				}
			});
	...
	asyncTest(
			"handle keyboard navigation on menu with scroll and with submenus",
			function() {
				expect(14);
				...
				function menukeyboard4() {
					...
					equal(logOutput(), "keydown,Aberdeen",
	...
	asyncTest(
			"handle keyboard navigation and mouse click on menu with disabled items",
			function() {
				expect(6);
				...
				element.one("menufocus", function() {
					...
					equal(logOutput(), "keydown,1",
							"Keydown focus but not select disabled item");
					setTimeout(menukeyboard1, 50);
				});
				element.focus();
				...
				function menukeyboard1() {
					...
					equal(logOutput(), "keydown,2,3,4",
							"Keydown focus disabled item with submenu");
					...
					setTimeout(
							function() {
								equal(logOutput(), "keydown",
										"Keydown RIGHT (no effect on disabled sub-menu)");
								...
								setTimeout(
										function() {
											equal(logOutput(), "keydown",
													"Keydown ENTER (no effect on disabled sub-menu)");
											log("click", true);
											click(element, "1");
											equal(logOutput(), "click",
													"Click disabled item (no effect)");
											start();
										}, 50);
							}, 50);
				}
				...
				element
						.one(
								...
								function() {
									...
									equal(logOutput(), "keydown,0,1,3",
											"Keydown focus Addyston by spelling the first 3 letters");
									element.simulate("keydown", {
										keyCode : 68
									});
									equal(logOutput(), "keydown,0,1,3,4",
											"Keydown focus Delphi by repeating the 'd' again");
									start();
								});
				element[0].focus();
			});
